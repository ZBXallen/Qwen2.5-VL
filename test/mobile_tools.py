...
""".strip(),
    ...
    def _key(self, text: str):
        common = f'input keyevent {text}'
        self.device.shell(common)
        
    def _click(self, coordinate: Tuple[int, int]):
        x, y = coordinate
        common = f'input tap {x} {y}'
        self.device.shell(common)

    def _long_press(self, coordinate: Tuple[int, int], time: int):
        x, y = coordinate
        common = f'input swipe {x} {y} {x} {y} {time * 1000}'
        self.device.shell(common)

    def _swipe(self, coordinate: Tuple[int, int], coordinate2: Tuple[int, int]):
        x1, y1 = coordinate
        x2, y2 = coordinate2
        common = f'input swipe {x1} {y1} {x2} {y2}'
        self.device.shell(common)

    def _type(self, text: str):
        common = f'input text {text}'
        self.device.shell(common)
    ...
    def _system_button(self, button: str):
        ...
        common = f'input keyevent {button_map[button]}'
        self.device.shell(common)

    def _open(self, text: str):
        common = f'monkey -p {text} -c android.intent.category.LAUNCHER 1'
        self.device.shell(common)
